...
async function handleCreateRace() {
	...
	let race = await createRace(player_id,track_id);//race 
...
function runRace(raceID) {
	...
	*/else if(res.status==="finished"){
		clearInterval(gameInterval) // to stop the interval from repeating
...
//to handle error in <h1> I create this method :
function h1_race(track){
if (track)
return track.name; 
return' ';
}
...
function raceProgress(positions) {
	let userPlayer = positions.find(e => e.id === store.player_id -1)
	userPlayer.driver_name += " (me)"
...
async function getTracks() {
	...
	.then(res => res.json())
	.catch(err => console.log(" In getTracks ", err))
...
async function getRacers() {
	...
	.then(res => res.json())
	.catch(err => console.log(" In getRacers ", err))
